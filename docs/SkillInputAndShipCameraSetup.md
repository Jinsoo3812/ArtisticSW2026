...
## 대포 조작 및 사수 위치 (Player Mount / Exit Point) 설정

`ACannon`에는 사수가 대포에 탑승했을 때 위치할 포인트를 제어하는 두 개의 씬 컴포넌트가 포함되어 있습니다.

| 컴포넌트 이름 | 부모 컴포넌트 | 기본 상대 위치 | 설명 |
| --- | --- | --- | --- |
| `PlayerMountPoint` | `BaseMesh` (Yaw 회전축) | `(-120, 0, 0)` | 탑승 시 캐릭터가 스냅되어 서 있는 위치. 대포 좌우 조준 회전 시 함께 회전합니다. |
| `PlayerExitPoint` | `RootComponent` (고정) | `(-140, 0, 0)` | 조준 모드 종료(`ESC` / `F`) 시 캐릭터가 복귀할 갑판 위치. |

- **대포 배치 독립성**: `PlayerMountPoint`는 대포 자체의 로컬 좌표계(`-X`, 포신 반대편)를 기준으로 하므로, 배의 좌현/우현 어느 각도에 대포가 배치되어 있어도 자동으로 배 안쪽 갑판 방향을 정확히 조준 위치로 인식합니다.
- **에디터 미세 조정**: `BP_Cannon` 블루프린트 뷰포트에서 `PlayerMountPoint`와 `PlayerExitPoint`의 트랜스폼을 선택하여 원하는 대포 손잡이 위치로 자유롭게 드래그해 조정할 수 있습니다.
