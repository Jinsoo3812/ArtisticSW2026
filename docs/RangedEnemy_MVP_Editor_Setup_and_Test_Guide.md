...
> **리팩터링 안내 (2026-08-03):** 이 문서의 Controller 10Hz 자동 사격 루프와
> "Behavior Tree를 지정하지 않는다" 설정은 초기 고정형 MVP 기록이다. 현재 지상 전투용
> RangedEnemy는 State 기반 BT를 사용한다. 새 설정은
> `docs/Enemy_AI_State_Perception_BT_Refactor_Guide.md`를 기준으로 한다.
...
1. `ABaseAIController::OnTargetPerceptionUpdated`
   - `HasAuthority()`가 true인지 확인한다.
   - Sight Stimulus의 `WasSuccessfullySensed()`가 true인지 확인한다.
   - 처리 후 Blackboard의 `State=Combat`, `TargetActor=Player`인지 확인한다.
2. Ranged Ground Combat Subtree
   - `Can Ranged Attack` Decorator가 TargetActor, 사거리, LOS를 통과하는지 확인한다.
   - `Ranged Attack` Task가 실행되는지 확인한다.
3. `ARangedEnemy::IsValidCombatTarget`
...
4. `ARangedEnemy::CanAttackTarget`
...
5. `ARangedEnemy::TryStartRangedAttack`
...
6. `UGA_RangedEnemyAttack::ActivateAbility`
...
7. 첫 격리 테스트에서는 `Attack Montage`를 비운다. 이 경우 GA가 성공하면 즉시 Projectile을 생성한다. 즉시 발사는 되지만 Montage를 지정하면 안 된다면 AnimBP Slot과 `Event.Montage.FireArrow` Notify 설정을 조사한다.
